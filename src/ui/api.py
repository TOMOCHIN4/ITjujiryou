...
# events を DB から poll して broker に流すバックグラウンドタスク。
# マルチプロセス構成では各 MCP サーバプロセスが in-memory broker に publish しても
# FastAPI プロセスの broker には届かないため、DB を経由してファンアウトする。
EVENT_PUMP_INTERVAL_S = 0.5
EVENT_PUMP_BATCH_LIMIT = 200


async def _event_pump() -> None:
    store = get_store()
    last_id = await store.last_event_id()
    while True:
        try:
            # list_events は DESC 順なので時系列順 (ASC) に流す
            events = await store.list_events(since_id=last_id, limit=EVENT_PUMP_BATCH_LIMIT)
            for ev in reversed(events):
                broker.publish({"type": "event", **_event_to_payload(ev)})
                if ev.get("id", 0) > last_id:
                    last_id = ev["id"]
        except Exception:
            # poll の一時エラーで pump を止めない
            pass
        await asyncio.sleep(EVENT_PUMP_INTERVAL_S)
